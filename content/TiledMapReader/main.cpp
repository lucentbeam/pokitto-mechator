...
void appendLevelElements(std::ofstream &door_file, std::ofstream &file, nlohmann::json &json, int dx, int dy) {
    for (int i = 0; i < json["objects"].size(); i++) {
        ...
        if (name[0] == "hackingkit") {
            ...
            if (name[1] == "a") {
                door_file << leading << "spawnDoorA<" << x << "," << y << "," << w << "," << h << "," << t << ">),\n";
            } else if (name[1] == "b") {
                door_file << leading << "spawnDoorB<" << x << "," << y << "," << w << "," << h << "," << t << ">),\n";
            } else if (name[1] == "c") {
                door_file << leading << "spawnDoorC<" << x << "," << y << "," << w << "," << h << "," << t << ">),\n";
            } else if (name[1] == "none") {
                file << leading << "spawnDoorNone<" << x << "," << y << "," << w << "," << h << "," << t << ">),\n";
            }
            ...
            if (name[1] == "mech") {
                ...
                leading += "}, ";
                if (name[2] == "ns") {
                    file << leading << "spawnVerticalLasers<" << h <<">),\n";
                } else {
                    file << leading << "spawnHorizontalLasers<" << w <<">),\n";
                }
            } else if (name[1] == "heli") {
                ...
                int h = json["objects"][i]["height"].get<float>() / 6.0f;
                h++;
                file << leading << "spawnWaterMine<" << w << ", " << h << ">),\n";
            } else if (name[1] == "boat") {
                file << leading << "Enemy::spawnBoat),\n";
            }
