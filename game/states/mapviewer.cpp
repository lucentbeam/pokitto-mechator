...
void MapViewer::draw()
{
    ...
    map_area.draw(true, [](int16_t x, int16_t y, int16_t w, int16_t h){
        ...
        if (h > 56) {
            ...
            const int doors[3] = {49, 13, 43}; // blue, red, green
            int color;
            for (int i = 0; i < spawnpoint_data_count; ++i) {
                if (GameVariables::doorStates()[i] == Discovered) {
                    SpawnPoint pt = points[i];
                    color = pt.type() == SpawnPoint::SpawnPointType::DoorA ? doors[0] : pt.type() == SpawnPoint::SpawnPointType::DoorB ? doors[2] : pt.type() == SpawnPoint::SpawnPointType::DoorC ? doors[1] : 10;
            ...
            if (goal.x() > -200) {
                ...
                RenderSystem::sprite(tl.x() + pos.x() - 3.5f, tl.y() + pos.y() - 3.5f, poi[goal_blink.active() ? 1 : 3], 0);
            }
            Helpers::drawNotchedRect(80, y + h/2 - 17, 30, 37, 0);
            if (door_blink.active()) {
                RenderSystem::drawRect2(83, y + h/2 + 5, 3, 3, doors[0]);
            } else if (door_blink.active(1.2f)) {
                RenderSystem::drawRect2(83, y + h/2 + 5, 3, 3, doors[1]);
            } else {
                RenderSystem::drawRect2(83, y + h/2 + 5, 3, 3, doors[2]);
            }
            RenderSystem::pixel(84, y + h/2 - 12, playerloc_blink.active() ? 10 : 5);
            RenderSystem::print(91, y + h/2 - 15, "loc", 10);

            RenderSystem::sprite(81, y + h/2 - 6, poi[goal_blink.active() ? 1 : 3], 0);
            ...
            RenderSystem::pixel(tl.x() + pos.x(), tl.y() + pos.y(), playerloc_blink.active() ? 10 : 5);
        }
    });
