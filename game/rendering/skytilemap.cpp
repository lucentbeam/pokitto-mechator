...
{
    int16_t x = std::floor(Camera::draw_x()/18.0f);
    int16_t y = std::floor(Camera::draw_y()/18.0f);
    int sx = int(x) * 18 - Camera::draw_x();
    int sy = int(y) * 18 - Camera::draw_y();
    int16_t x_upper = x + render_width;
    if (x_upper > m_mapwidth) x_upper = m_mapwidth;
    int16_t y_upper =y +render_height;
    if (y_upper > m_mapheight) y_upper = m_mapheight;
    int16_t x_lower = x < 0 ? 0 : x;
    if (x_lower >= x_upper) return;
    int16_t y_lower = y < 0 ? 0 : y;
    ...
    for(int16_t j = y_lower; j < y_upper; j++) {
        ...
        for(int16_t i = x_lower; i < x_upper; ++i) {
            if (current_tile != uint8_t(-1)) {
                RenderSystem::drawShadow(sx + (i-x) * 18, sy + (j-y) * 18 + 3, m_tiles[current_tile], m_tiles[current_tile][2]);
                RenderSystem::sprite(sx + (i-x) * 18, sy + (j-y) * 18, m_tiles[current_tile], m_tiles[current_tile][2]);
            }
            counter--;
            if (counter <= 0) {
                tile += 2;
