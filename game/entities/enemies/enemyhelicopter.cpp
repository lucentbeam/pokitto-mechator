...
void EnemyHelicopter::draw() const
{
    ...
    RenderSystem::drawShadow(pos.x(), pos.y() + 10, helicopter[m_steering.getRotationFrame(m_steering.aim(), 8.0f)], helicopter[0][2], m_steering.aim().x() > 0);
}
...
void EnemyHelicopter::drawAir() const
{
    ...
    if (m_damage_frames > 0) {
        RenderSystem::sprite(pos.x(), pos.y(), helicopter[m_steering.getRotationFrame(m_steering.aim(), 8.0f)], helicopter[0][2], 10, m_steering.aim().x() > 0);
        ...
        RenderSystem::sprite(pos.x(), pos.y(), helicopter[m_steering.getRotationFrame(m_steering.aim(), 8.0f)], helicopter[0][2], m_steering.aim().x() > 0);
